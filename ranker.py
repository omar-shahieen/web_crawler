...
def _build_link_graph() -> tuple[Dict[ObjectId, Set[ObjectId]], Dict[ObjectId, float]]:
    ...
    url_to_id: Dict[str, ObjectId] = {p["url"]: p["_id"] for p in pages if p.get("url")}
    adjacency: Dict[ObjectId, Set[ObjectId]] = {}

    for p in pages:
        src = p["_id"]
        links = p.get("out_links", [])
    ...
    n = len(adjacency)
    base = 1.0 / n
    ranks: Dict[ObjectId, float] = {doc_id: base for doc_id in adjacency}
...
def _compute_pagerank_scores(iterations: int = 20, damping: float = 0.85) -> Dict[ObjectId, float]:
    ...
    for _ in range(iterations):
        ...
        for src, outs in adjacency.items():
            ...
            share = damping * src_rank / len(outs)
            for dst in outs:
                new_ranks[dst] += share

        # Distribute sink pages equally.
    ...
    # Normalize popularity to [0, 1] for stable weighted combination.
...
def combine_scores(
    ...
    for doc_id, rel in relevance_scores.items():
